...
const Dashboard = () => {
  ...
  const navItems = [
    ...
    {
      label: "My Resume",
      path: "/dashboard/jobseeker/profile/edit",
      Icon: FiFileText,
    },
    {
      label: "Settings",
      path: "/dashboard/jobseeker/settings",
      Icon: FiSettings,
    },
  ];
  ...
  const handleApplyFilters = async (event) => {
    ...
    try {
      ...
      const jobs = response.data.filter((job) => {
        ...
        const skills = (Array.isArray(job.skills) ? job.skills : []).map(
          (skill) => skill.toLowerCase(),
        );
        const workMode = String(
          job.workMode || (jobType === "remote" ? "Remote" : "On-site"),
        ).toLowerCase();
        const salaryValues =
          String(job.salary || "")
            .match(/[\d,.]+/g)
        ...
        return (
          (!jobFilters.jobTypes.length ||
            jobFilters.jobTypes.some(
              (type) => jobType === type.toLowerCase(),
            )) &&
          (!jobFilters.experiences.length ||
            jobFilters.experiences.some(
              (value) => experience === value.toLowerCase(),
            )) &&
          (!jobFilters.workModes.length ||
            jobFilters.workModes.some(
              (value) => workMode === value.toLowerCase(),
            )) &&
          (!jobFilters.skills.length ||
            jobFilters.skills.every((skill) =>
      ...
      setMatchingJobs([]);
      setJobsError(
        getErrorMessage(requestError, "Unable to load matching jobs"),
      );
    } finally {
    ...
    return (
      <div className="min-h-screen bg-gray-50 p-6">
        ...
        <p className="mt-3 text-gray-600">
          Please log in to view your profile and dashboard.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="h-screen overflow-hidden bg-[#eaf4f2] text-[#132238]">
      <div className="mx-auto flex h-screen max-w-[1600px] overflow-hidden">
        <aside className="w-[290px] shrink-0 overflow-y-auto bg-[#132238] px-6 py-8 text-white shadow-[10px_0_30px_rgba(19,34,56,0.15)]">
        ...
        <main className="min-w-0 flex-1 overflow-y-auto bg-[#f4f7f8] p-6 md:p-8">
          {activeSection === "jobs" && (
            <div className="mb-8 rounded-[2rem] bg-white p-6 shadow-[0_15px_35px_rgba(19,34,56,0.08)] ring-1 ring-slate-200">
              ...
              <h1 className="mt-2 text-4xl font-black tracking-tight text-[#132238]">
                Find your next opportunity.
              </h1>
              <p className="mt-2 text-slate-500">
                Use the filters to discover roles that match your goals.
              </p>
            </div>
          )}
          ...
          {activeSection === "jobs" && (
            <div
              id="find-jobs"
              className="mb-8 scroll-mt-6 rounded-[2rem] bg-white p-6 shadow-[0_15px_35px_rgba(19,34,56,0.08)] ring-1 ring-slate-200"
            >
              <form
                onSubmit={handleApplyFilters}
                className="grid gap-6 rounded-[1.5rem] bg-[#f5f7fb] p-5 lg:grid-cols-[235px_1fr]"
              >
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-500">
                    Filter opportunities that match your goals.
                  </p>
                </div>
                ...
                <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
                  <label className="text-sm font-bold text-[#132238]">
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        updateJobFilter("keyword", event.target.value)
                      }
                      placeholder="React Developer"
                      className="mt-2 w-full rounded-xl border border-slate-200 bg-white px-4 py-3 font-normal outline-none ring-[#69d4cf] placeholder:text-slate-400 focus:ring-4"
                    />
                  </label>
                  ...
                  <FilterGroup
                    ...
                    selected={jobFilters.jobTypes}
                    onToggle={(value) => toggleJobFilter("jobTypes", value)}
                  />
                  <FilterGroup
                    title="Experience"
                    options={["Fresher", "1-2 Years", "3-5 Years", "5+ Years"]}
                    selected={jobFilters.experiences}
                    onToggle={(value) => toggleJobFilter("experiences", value)}
                  />

                  <fieldset>
                    <legend className="text-sm font-bold text-[#132238]">
                      Salary
                    </legend>
                    <div className="mt-2 flex gap-2">
                      <input
                        ...
                        onChange={(event) =>
                          updateJobFilter("minSalary", event.target.value)
                        }
                        placeholder="Min"
                        className="w-full rounded-xl border border-slate-200 bg-white px-3 py-3 text-sm outline-none ring-[#69d4cf] focus:ring-4"
                      />
                      <input
                        type="number"
                        min="0"
                        ...
                        onChange={(event) =>
                          updateJobFilter("maxSalary", event.target.value)
                        }
                        placeholder="Max"
                        className="w-full rounded-xl border border-slate-200 bg-white px-3 py-3 text-sm outline-none ring-[#69d4cf] focus:ring-4"
                      />
                  ...
                  <FilterGroup
                    ...
                    selected={jobFilters.workModes}
                    onToggle={(value) => toggleJobFilter("workModes", value)}
                  />
                  <FilterGroup
                    title="Skills"
                    options={["React", "Node.js", "MongoDB", "JavaScript"]}
                    selected={jobFilters.skills}
                    onToggle={(value) => toggleJobFilter("skills", value)}
                  />

                  <button
                    type="submit"
                    className="self-end rounded-xl bg-[#132238] px-5 py-3 text-sm font-black text-white transition hover:bg-[#0d9f9a]"
                  >
                    Apply Filters
                  </button>
                </div>
              ...
              {hasAppliedFilters && (
                <section className="mt-8">
                  <div className="mb-5 flex items-center justify-between gap-4">
                    <div>
                      ...
                      {!isLoadingJobs && !jobsError && (
                        <p className="mt-1 text-sm text-slate-500">
                          {matchingJobs.length} jobs match your filters.
                        </p>
                      )}
                    </div>
                  </div>
                  ...
                  {jobsError && (
                    <p className="rounded-2xl border border-red-200 bg-red-50 p-5 text-red-600">
                      {jobsError}
                    </p>
                  )}
                  {!isLoadingJobs &&
                    ...
                    matchingJobs.length === 0 && (
                      <p className="rounded-2xl border border-slate-200 bg-slate-50 p-5 text-slate-500">
                        No jobs match these filters. Try changing your search
                        criteria.
                      </p>
                    )}
            ...
            <>
              <div className="mb-8 rounded-[2rem] bg-white p-6 shadow-[0_15px_35px_rgba(19,34,56,0.08)] ring-1 ring-slate-200">
                <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
                  <div>
                    <p className="flex items-center gap-2 text-3xl font-bold text-[#132238]">
                      Good Morning, {currentUser?.name?.split(" ")[0] || "John"}{" "}
                      <FaHandPaper className="text-[#e07a45]" />
                    </p>
                    <h1 className="mt-2 text-4xl font-black tracking-tight text-[#132238]">
                      Find your next opportunity.
                    </h1>
              ...
              <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
                {stats.map(({ label, value, Icon }) => (
                  <div
                    key={label}
                    className="rounded-[1.6rem] border border-slate-200 bg-white p-5 shadow-[0_12px_25px_rgba(19,34,56,0.05)]"
                  >
                    <div className="flex items-center justify-between">
                      <Icon className="text-2xl text-[#0d9f9a]" />
                      <span className="rounded-full bg-[#e8f9f8] px-2 py-1 text-xs font-semibold text-[#0d9f9a]">
                        Live
                      </span>
                    </div>
                    ...
                    <h2 className="mt-3 text-[2rem] font-black leading-none text-[#132238]">
                      {value}
                    </h2>
                  </div>
                ))}
              ...
              <div className="mt-8 rounded-[2rem] bg-white p-6 shadow-[0_12px_25px_rgba(19,34,56,0.05)] ring-1 ring-slate-200">
                ...
                <div className="rounded-[1.5rem] border border-slate-200 bg-[#f9fbfd] p-5">
                  <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    <div>
                      <h3 className="text-2xl font-bold text-[#132238]">
                        Frontend Developer
                      </h3>
                  ...
                  <div className="mt-5 flex flex-wrap items-center gap-5 text-sm text-slate-600">
                    <span className="inline-flex items-center gap-1.5">
                      <FiMapPin className="text-[#0d9f9a]" /> Ahmedabad
                    </span>
                    <span className="inline-flex items-center gap-1.5">
                      <FiDollarSign className="text-[#0d9f9a]" /> ₹5 - ₹8 LPA
                    </span>
                  </div>

                  <div className="mt-6 flex items-center justify-between gap-3 border-t border-slate-200 pt-4">
                    <div className="flex gap-2">
                      <span className="rounded-full bg-[#e6f7f5] px-3 py-1 text-xs font-semibold text-[#0d9f9a]">
                        React
                      </span>
                      <span className="rounded-full bg-[#eef3ff] px-3 py-1 text-xs font-semibold text-[#4f6ef7]">
                        JavaScript
                      </span>
                    </div>
                    ...
                    <button
                      type="button"
                      className="rounded-xl bg-[#132238] px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-[#1d3354]"
                    >
                      View Job
                    </button>
                  </div>
      ...
      {options.map((option) => (
        ...
        >
          <input
            ...
            className="h-4 w-4 accent-[#0d9f9a]"
          />
          {option}
        </label>
      ))}
