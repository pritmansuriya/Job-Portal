          ...
          <nav className="space-y-2">
            {navItems.map(([label, path, Icon]) => (
              <Link
                key={label}
                to={path}
                className={`flex items-center gap-3 rounded-2xl px-4 py-3 text-sm font-medium transition hover:bg-white/10 hover:text-white ${label === "My Profile" ? "bg-white/10 text-white" : "text-slate-200"}`}
              >
          ...
          <div className="mx-auto max-w-5xl">
            ...
            <section className="rounded-[2rem] bg-white p-6 shadow-[0_15px_35px_rgba(19,34,56,0.08)] md:p-8">
              <div className="flex flex-col items-center text-center">
                <div className="flex h-24 w-24 items-center justify-center rounded-full bg-[#0d9f9a] text-4xl font-black text-white shadow-[5px_5px_0_#f6c453]">
                  {profile.name?.charAt(0)?.toUpperCase() || "J"}
                </div>
                <h2 className="mt-5 text-3xl font-black">
                  {profile.name || "John Doe"}
                </h2>
                <p className="mt-1 text-lg font-semibold text-[#0d9f9a]">
              ...
              <div className="mt-8 grid gap-4 border-t border-slate-200 pt-6 sm:grid-cols-3">
                <ContactItem
                  icon={FiMail}
                  value={profile.email || lastUser?.email || "No email"}
                />
                <ContactItem
                  icon={FiMapPin}
              ...
              <div className="mt-8">
                <div className="flex items-center justify-between text-sm font-bold">
                  ...
                  <span className="text-[#0d9f9a]">{completion}%</span>
                </div>
                <div className="mt-3 h-3 overflow-hidden rounded-full bg-slate-100">
                  <div
                    className="h-full rounded-full bg-[#0d9f9a] transition-all"
            ...
            <ProfileSection title="About Me">
              <p className="leading-8 text-slate-600">
                {profile.bio ||
                  "Add a short introduction about your experience, goals, and the work you enjoy."}
              </p>
            </ProfileSection>
            ...
            <ProfileSection title="Experience">
              <div className="border-l-4 border-[#0d9f9a] pl-5">
                ...
                <p className="mt-1 text-sm text-slate-500">
                  {profile.experience
                    ...
                    : "Add your company and dates from Edit Profile"}
                </p>
              </div>
            </ProfileSection>
            <ProfileSection title="Education">
              <p className="text-lg font-bold text-[#132238]">
            ...
            <ProfileSection title="Projects">
              <p className="whitespace-pre-line leading-8 text-slate-600">
                {profile.projects ||
                  "Add your projects, responsibilities, and technologies used."}
              </p>
            </ProfileSection>
            ...
            <ProfileSection title="Languages">
              <p className="leading-8 text-slate-600">
                {profile.languages || "Add the languages you speak."}
              </p>
            </ProfileSection>
            <ProfileSection title="Achievements">
              <p className="whitespace-pre-line leading-8 text-slate-600">
                {profile.achievements || "Add your awards and achievements."}
              </p>
            </ProfileSection>
            <ProfileSection title="Resume">
              <div className="flex flex-col gap-4 rounded-2xl bg-[#f5f7fb] p-5 sm:flex-row sm:items-center sm:justify-between">
                <div className="flex items-center gap-3">
                  <FiFileText className="text-2xl text-[#0d9f9a]" />
                ...
                {profile.resume && (
                  <div className="flex gap-3">
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="rounded-xl border border-[#0d9f9a] px-4 py-2 text-sm font-bold text-[#087b78]"
                    >
                      View
                    </a>
                    <a
                      href={profile.resume}
                      download
                      className="rounded-xl bg-[#132238] px-4 py-2 text-sm font-bold text-white"
                    >
                      Download
                    </a>
                  </div>
...
const ContactItem = ({ icon: Icon, value }) => (
  <div className="flex items-center gap-3 text-sm text-slate-600">
    <Icon className="text-xl text-[#0d9f9a]" />
    <span>{value}</span>
  </div>
);
const ProfileSection = ({ title, children }) => (
  <section className="mt-6 rounded-[1.7rem] bg-white p-6 shadow-[0_12px_25px_rgba(19,34,56,0.05)] md:p-7">
    <h2 className="mb-5 border-b border-slate-200 pb-4 text-2xl font-black">
      {title}
    </h2>
    {children}
  </section>
