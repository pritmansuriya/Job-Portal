  ...
  return (
    <div className="min-h-screen bg-[#eaf4f2] text-[#132238]"><div className="mx-auto flex min-h-screen max-w-[1600px]">
      <aside className="hidden w-[290px] shrink-0 bg-[#132238] px-6 py-8 text-white shadow-[10px_0_30px_rgba(19,34,56,0.15)] md:block">
        ...
        <nav className="space-y-2">{navItems.map(([label, path, Icon]) => <Link key={label} to={path} className={`flex items-center gap-3 rounded-2xl px-4 py-3 text-sm font-medium transition hover:bg-white/10 hover:text-white ${label === "My Profile" ? "bg-white/10 text-white" : "text-slate-200"}`}><Icon className="shrink-0 text-lg" /><span>{label}</span></Link>)}<button type="button" onClick={handleLogout} className="mt-6 flex w-full items-center gap-3 rounded-2xl bg-red-500 px-4 py-3 text-left text-sm font-semibold text-white transition hover:bg-red-600"><FiLogOut className="shrink-0 text-lg" /><span>Logout</span></button></nav>
      </aside>
      <main className="min-w-0 flex-1 px-6 py-10 md:px-10"><div className="mx-auto max-w-5xl">
        ...
        <section className="rounded-[2rem] bg-white p-6 shadow-[0_15px_35px_rgba(19,34,56,0.08)] md:p-8">
          ...
          <div className="mt-8 grid gap-4 border-t border-slate-200 pt-6 sm:grid-cols-3"><ContactItem icon={FiMail} value={profile.email || lastUser?.email || "No email"} /><ContactItem icon={FiMapPin} value={profile.location || "Location not added"} /><ContactItem icon={FiPhone} value={profile.phone || "Phone not added"} /></div>
        ...
        <ProfileSection title="Resume"><div className="flex flex-col gap-4 rounded-2xl bg-[#f5f7fb] p-5 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><FiFileText className="text-2xl text-[#0d9f9a]" /><span className="font-bold">{profile.resume || "No resume uploaded"}</span></div>{profile.resume && <div className="flex gap-3"><a href={profile.resume} target="_blank" rel="noreferrer" className="rounded-xl border border-[#0d9f9a] px-4 py-2 text-sm font-bold text-[#087b78]">View</a><a href={profile.resume} download className="rounded-xl bg-[#132238] px-4 py-2 text-sm font-bold text-white">Download</a></div>}</div></ProfileSection>
      </div></main>
    </div></div>
  );
};

const ContactItem = ({ icon: Icon, value }) => <div className="flex items-center gap-3 text-sm text-slate-600"><Icon className="text-xl text-[#0d9f9a]" /><span>{value}</span></div>;
